.. CosmoTransitions documentation master file, created by
   sphinx-quickstart on Sat Feb  8 11:35:32 2014.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

Welcome to CosmoTransitions's documentation!
============================================

.. image:: deform1.jpg
   :align: center

CosmoTransitions is a package for analyzing finite or zero-temperature cosmological phase transitions driven by single or multiple scalar fields. It is written and maintained by Carroll L. Wainwright (a.k.a. "Max") (cwainwri@ucsc.edu), and is free for public use. If you use CosmoTransitions in scholarly work, please cite `Comput. Phys. Commun. 183 (2012)`_ [`arXiv:1109.4189`_].

Note that the current version (2.0a1) should be considered a development build. There will be bugs that I have not yet found or tested for. If you happen to find any, let me know! Or, better yet, fork the CosmoTransitions git repository and contribute directly to the code.

 .. _`arXiv:1109.4189`: http://arxiv.org/abs/1109.4189
 .. _`Comput. Phys. Commun. 183 (2012)`: http://dx.doi.org/10.1016/j.cpc.2012.04.004

Contents
========

.. toctree::
   :maxdepth: 2

   Overview
   tunnelling1D
   pathDeformation
   transitionFinder
   generic_potential
   finiteT
   helper_functions
   multi_field_plotting


Indices and tables
==================

* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`
